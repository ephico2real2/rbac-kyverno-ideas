...
echo -e "${BLUE}=== Setting up Test Namespaces for Kyverno RBAC Testing (v2-numbered labels) ===${NC}"
...
setup_namespace_configs() {
    ...
    # multi-admin-test (multiple groups per role type - demonstrates v2-numbered power)
    create_namespace_with_labels "multi-admin-test" "app-ocp-rbac-security-ns-admin,app-ocp-rbac-platform-ns-admin,app-ocp-rbac-devops-ns-admin,app-ocp-rbac-frontend-ns-developer,app-ocp-rbac-backend-ns-developer,app-ocp-rbac-security-ns-audit,app-ocp-rbac-compliance-ns-audit,app-ocp-rbac-monitoring-ns-audit"
}
...
create_namespace_with_labels() {
    ...
    # Parse groups and apply numbered labels
    IFS=',' read -ra GROUP_ARRAY <<< "$groups"
    
    # Initialize counters for each role
    local admin_counter=1
    local edit_counter=1
    local view_counter=1
    
    # Apply RBAC labels based on group suffixes using numbered approach
    for group in "${GROUP_ARRAY[@]}"; do
        if [[ $group == *"-admin" ]]; then
            kubectl label namespace "$ns_name" "oim-ns-admin-${admin_counter}"="$group" --overwrite
            echo -e "${GREEN}✓ Added admin label: oim-ns-admin-${admin_counter}=$group${NC}"
            ((admin_counter++))
        elif [[ $group == *"-developer" ]]; then
            kubectl label namespace "$ns_name" "oim-ns-edit-${edit_counter}"="$group" --overwrite
            echo -e "${GREEN}✓ Added edit label: oim-ns-edit-${edit_counter}=$group${NC}"
            ((edit_counter++))
        elif [[ $group == *"-audit" ]]; then
            kubectl label namespace "$ns_name" "oim-ns-view-${view_counter}"="$group" --overwrite
            echo -e "${GREEN}✓ Added view label: oim-ns-view-${view_counter}=$group${NC}"
            ((view_counter++))
        fi
    done
...
setup_control_examples() {
    ...
    if kubectl get namespace openshift-config >/dev/null 2>&1; then
        ...
        kubectl label namespace openshift-config oim-ns-admin-1=app-ocp-rbac-platform-ns-admin --overwrite
    ...
    kubectl label namespace kill-switch-test oim-ns-admin-1=app-ocp-rbac-demo-ns-admin --overwrite
...
verify_setup() {
    ...
    for ns in "demo-ns" "team-alpha-dev" "team-beta-prod" "shared-tools" "multi-admin-test" "kill-switch-test"; do
...
main() {
    ...
    echo "2. Apply Kyverno v2 policy: kubectl apply -f namespace-rolebinding-policy-v2-numbered.yaml"
